...
struct PanelExpandedView: View {
	...
	var body: some View {
		...
		.animation(
			.easeInOut(duration: UIDuration.panelSettingsToggle),
			value: panelController.isSettingsPresented
		)
	}

	private var settingsAccessory: some View {
		PanelSettingsAccessoryButton(
			isPresented: panelController.isSettingsPresented,
			onToggle: panelController.toggleSettings
		)
